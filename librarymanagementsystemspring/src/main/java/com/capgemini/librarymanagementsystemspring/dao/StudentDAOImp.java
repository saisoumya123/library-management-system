...
import com.capgemini.librarymanagementsystemspring.dto.RequestBookPK;
@Repository
	...
	public boolean req(int id, int book_id) {
		...
		try {
			...
			if(c != 0) {
				...
				Query req = manager.createNativeQuery("insert into requestbookspring (bid,email,book_title,id,name,type) values (?,?,?,?,?,?)");
